...
class Streamlit_UI:
    ...
    def display_main_UI(self):
        ...
        if song_name:
            songs = self._backend.read_music_db(song_name)
            st.write("### Search results: ")
            for _ , song in songs.iterrows():
                if(st.button(f"{song.TRACK_NAME} - {song.ARTIST_NAME}", key = song.TRACK_ID)):
                    st.session_state.search = song.to_dict()
                    st.rerun()
    ...
    """ ====================================== DISPLAY SONG ========================================="""
    def display_search_song(self):
        song = st.session_state.search
        ...
        recommend_songs = self._backend.exec_rcm_bcf(song['TRACK_ID'])
